...
fs.readdir(folder, 
    (err, files) => {
        if(err) {
            console.log(pc.magenta(`⚠️ No se pudo leer el directiorio ${folder}`));
            process.exit(1);
        }
        files.forEach(file => {
            ...
            fs.stat(filePath, 
                (err, stat)=> {
                    ...
                    console.log(`${pc.green(fileType)} ${pc.blue(filePath.padEnd(30))} ${fileSize.padStart(10)} ${modifiedDate}`);
            })
        });
})